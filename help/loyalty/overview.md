---
title: Prise en main de la fidélité Journey Optimizer pour les professionnels du marketing
description: Découvrez comment intégrer Adobe Journey Optimizer Loyalty, configurer un défi, l’appliquer et l’afficher, et analyser ses performances.
topic: Get Started
role: User
level: Beginner
doc-type: Tutorial
jira: KT-21773
last-substantial-update: 2026-07-28T00:00:00Z
source-git-commit: 7523b27390c4d3a5447a0ee340eee07086d22be8
workflow-type: tm+mt
source-wordcount: '384'
ht-degree: 0%

---


# Prise en main de la fidélité Journey Optimizer pour les professionnels du marketing

Cours de base pour les spécialistes du marketing sur l’intégration à Adobe Journey Optimizer Loyalty : configurez un défi de fidélité, appliquez-le et affichez-le, et analysez ses performances.

## Ce que vous apprenez

* Découvrez ce qu’est la fidélité à Journey Optimizer, comment elle se situe sous AJO et le cycle de vie des défis de bout en bout.
* Créez et configurez un défi de fidélité dans l’interface utilisateur de , y compris les tâches, les récompenses et l’agent Loyalty AI.
* Affichez et appliquez un défi par le biais d’une carte de contenu ou d’une expérience basée sur du code.
* Lisez le rapport de défi intégré pour suivre les performances.

### Présentation de la fidélité

Ce module présente Journey Optimizer Loyalty : ce qu’il est, où il se trouve sous Adobe Journey Optimizer et le cycle de vie des défis, de la configuration à l’analyse.

* [Découvrir ce qu’est la fidélité Journey Optimizer](./introduction-to-loyalty/discover-journey-optimizer-loyalty.md)
Découvrez ce qu’est la fidélité à Journey Optimizer, où elle se situe sous AJO et le cycle de vie des défis.

### Configurer votre défi

Ce module vous guide tout au long de la création et de la configuration d’un défi de fidélité de bout en bout : type, structure et planning, tâches et récompenses.

* [Configurer un défi de fidélité](./configure-your-challenge/set-up-a-loyalty-challenge.md)
Découvrez comment configurer un défi de fidélité en sélectionnant le type de défi approprié, en configurant des audiences et des plannings, en définissant des règles de participation et en contrôlant le suivi et la récompense de la progression.
* [Configurer des tâches pour votre défi de fidélité](./configure-your-challenge/create-tasks.md)
Découvrez comment configurer des tâches : achat et dépenses, quantités, articles et exclusions éligibles et réutilisation.
* [Configurer des récompenses pour votre défi de fidélité](./configure-your-challenge/configure-rewards.md)
Découvrez comment configurer des récompenses : fournisseur, jalon par rapport à la diffusion de fin, types de récompense et coupons.
* [Créez des défis et obtenez des informations avec l’agent Loyalty AI](./configure-your-challenge/create-challenges-and-get-insights-with-the-loyalty-ai-agent.md)
Découvrez comment utiliser l’agent Loyalty AI pour créer des défis et obtenir des informations.

### Déposez votre candidature et présentez votre défi

Ce module vous explique comment apporter un défi à vos clients à l’aide de cartes de contenu et d’expériences basées sur du code.

* [Création d’une carte de contenu type Défi avec une expérience basée sur du code](./apply-and-display-your-challenge/build-a-challenge-content-card-with-a-code-based-experience.md)
Découvrez comment créer une expérience basée sur une carte de contenu ou un code de défi, couvrant l’opt-in et la progression dynamique à travers l’opt-in, la progression et les étapes terminées, ainsi que les récompenses et la configuration des canaux.

### Analyse et rapport

Ce module explique comment suivre les performances de votre défi.

* [Suivre les performances avec le rapport de défi](./analyze-and-report/track-performance-with-the-challenge-report.md)
Découvrez comment suivre les performances de votre défi à l’aide du rapport de performance du défi.
