---
user-guide-title: Prise en main de la fidélité Journey Optimizer pour les professionnels du marketing
user-guide-description: Découvrez comment intégrer Adobe Journey Optimizer Loyalty, configurer un défi, l’appliquer et l’afficher, et analyser ses performances.
role: User
level: Beginner
breadcrumb-title: loyauté
breadcrumb-url: /docs/journey-optimizer-learn/loyalty/overview
auto-video-transcripts: true
source-git-commit: 7523b27390c4d3a5447a0ee340eee07086d22be8
workflow-type: tm+mt
source-wordcount: '108'
ht-degree: 0%

---


# Prise en main de la fidélité Journey Optimizer pour les professionnels du marketing {#loyalty}

+ [Prise en main de la fidélité Journey Optimizer pour les professionnels du marketing](./overview.md)
+ Présentation de la fidélité {#introduction-to-loyalty}
  + [Découvrez ce qu’est la fidélité à Journey Optimizer](./introduction-to-loyalty/discover-journey-optimizer-loyalty.md)
+ Configurer votre défi {#configure-your-challenge}
  + [Configurer un défi de fidélité](./configure-your-challenge/set-up-a-loyalty-challenge.md)
  + [Créer des tâches pour votre défi de fidélité](./configure-your-challenge/create-tasks.md)
  + [Configurer des récompenses pour votre défi de fidélité](./configure-your-challenge/configure-rewards.md)
  + [Créez des défis et obtenez des informations avec l’agent Loyalty AI](./configure-your-challenge/create-challenges-and-get-insights-with-the-loyalty-ai-agent.md)
+ Déposez votre candidature et présentez votre défi {#apply-and-display-your-challenge}
  + [Créer une carte de contenu de défi avec une expérience basée sur du code](./apply-and-display-your-challenge/build-a-challenge-content-card-with-a-code-based-experience.md)
+ Analyse et rapport {#analyze-and-report}
  + [Suivre les performances avec le rapport de défi](./analyze-and-report/track-performance-with-the-challenge-report.md)
